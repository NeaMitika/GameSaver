...
export default function App() {
  ...
  const { refreshGames, openDetail, refreshDetail, handleCreated, handleRemove, handleBackupNow, handleScanBackups, toggleLayoutMode } =
    useDashboardActions({
      isRecoveryMode,
      isScanningBackups,
      selectedDetail,
  ...
  useEffect(() => {
    ...
    window.gamesaver.windowControls.getLayoutMode().then(setLayoutMode).catch(() => undefined);
      ...
      <main className="mx-auto flex w-full flex-col gap-4 p-4">
        {isRecoveryMode && (
          <div className="flex flex-wrap items-center justify-between gap-3 rounded-md border border-destructive/40 bg-destructive/10 px-3 py-2 text-sm">
            <span>
              Recovery mode:{' '}
              {startupState.missingPath ? `Could not access ${startupState.missingPath}.` : 'Saved data path is unavailable.'}
            </span>
